...
int main(int ac, char *av[])
{
    ...
    */
    ...
    {
        sfile << "\n.STR" << i << ":\t.string " << _string_labels[i];
    }
    set<string> labelSet;
    globalSymbolTable->mark_labels();
    globalSymbolTable->globalVar(sfile);
    ...
    for (int i = 0; i < globalSymbolTable->symbolTabList.size(); ++i)
    {
        if (((globalSymbolTable->symbolTabList[i])->nested) != NULL)
        {
            if (labelSet.find(((globalSymbolTable->symbolTabList[i])->nested)->name) == labelSet.end())
            {
                ...
                globalSymbolTable->symbolTabList[i]->nested->function_prologue(sfile, count_l);
                globalSymbolTable->symbolTabList[i]->nested->destroyFunction(sfile);
                globalSymbolTable->symbolTabList[i]->nested->generateTargetCode(sfile, count_l);
                labelSet.insert(((globalSymbolTable->symbolTabList[i])->nested)->name);
                globalSymbolTable->symbolTabList[i]->nested->function_epilogue(sfile, count_l, count_l);
                count_l++;
            }
        }
    }
