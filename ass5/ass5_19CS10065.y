...
%union {                                                     // yylval is represented union of all these types

    char unaryOp;                                            // unaryoperator		
    char* char_value;                                        // char value
    int intval;                                              // integer value
    int instr_number;                                        // for backpatching
...
M: %empty 
	{
		//mainly used in backpatching,stores the next quad to be generated and used in control flow
		$$ = nextinstr();
	}   
	;
...
X: %empty 
	{
		/**
		  * change the current symbol pointer used for nested blocks
		  */
...
jump_statement: GOTO IDENTIFIER SEMICOLON { 
	...
	{
		$$ = new Statement();	
		emit("return",$2->loc->name);               //emit return with the name of the value returned
	}
	| RETURN SEMICOLON 
	{
		$$ = new Statement();	
...
function_definition:declaration_specifiers declarator declaration_list_opt changetable CURLY_BRACKET_OPEN block_item_list_opt CURLY_BRACKET_CLOSE  
	{
		...
		table_count = 0;
		label_table.clear();
		changeTable(globalST);                     //curly bracket close represents going back a symbol table
	}
	;
...
void yyerror(string s) {        //syntax error found
    cout<<s<<endl;
}